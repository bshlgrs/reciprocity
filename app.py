...
def get_current_user(access_token):
    me_info = requests.get(f"https://graph.facebook.com/v9.0/me?access_token={access_token}&fields=id,name,email").json()
    my_fb_id = me_info['id']
...
def api_info():
    ...
    me_info = requests.get(f"https://graph.facebook.com/v9.0/me?access_token={access_token}&fields=name,id,picture").json()
    my_fb_id = me_info['id']
    ...
    friend_objects = ses.query(User).filter(User.fb_id.in_([x['id'] for x in friends])).order_by(User.id).all()
    return jsonify([current_user, friend_objects, friend_pictures, my_checks, reciprocations, me_info['picture']['data']['url']])
